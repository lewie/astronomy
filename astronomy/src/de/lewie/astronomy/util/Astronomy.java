...
public class Astronomy {
    ...
    public static String sign(double lon) {
        String[] signs = { "Widder", "Stier", "Zwillinge", "Krebs", "Löwe", "Jungfrau", "Waage", "Skorpion", "Schütze",
    ...
    public static Coor observer2EquCart(double lon, double lat, double height, double gmst) {
        ...
        cart = equPolar2Cart(cart.x, cart.y, radius); // convert from geocentric polar to geocentric cartesian, with
                                                      // regard to Greenwich
    ...
    private static Coor sunPosition(double TDT, double geolat, double lmst, boolean useGeo) {
        ...
        sunCoor.sign = Astronomy.sign(sunCoor.lon);
        return sunCoor;
    }
    ...
    private static Coor moonPosition(Coor sunCoor, double TDT, Coor observer, double lmst, boolean useObs) {
        ...
        // String[] phases_EN = { "New moon", "Increasing crescent", "1st quarter", "Increasing Moon", "Full moon",
        // "Decreasing Moon", "Last quarter", "decreasing crescent", "New moon" };
        String[] phases_DE = { "Neumond", "Zunehmende Sichel", "Erstes Viertel", "Zunehmender Mond", "Vollmond",
                "Abnehmender Mond", "Letztes Viertel", "Abnehmende Sichel", "Neumond" };
     ...
     * @return Greenwich sidereal time (hours) of time of rise and set of object with coordinates Coor.ra/Coor.dec at
     *         geographic position lon/lat (all values in radians)
     */
    private static Riseset gMSTRiseSet(Coor corr, double lon, double lat, double h) {

        double tagbogen = Math
                .acos((Math.sin(h) - Math.sin(lat) * Math.sin(corr.dec)) / (Math.cos(lat) * Math.cos(corr.dec)));
    ...
    private static Riseset riseSet(double jd0UT, Coor coor1, Coor coor2, double lon, double lat, double timeinterval,
            double altitude, boolean useAlt) {
        ...
        if (!useAlt && altitude == 0.0) {
            alt = 0.5 * coor1.diameter - coor1.parallax + 34.0 / 60 * DEG;
        }
    ...
    public static Riseset moonRise(double JD, double deltaT, double lon, double lat, double zone, boolean recursive) {
        ...
        if (!recursive) { // check and adjust to have rise/set time on local calendar day
            ...
            if (rise.rise != 0.000000000) {
                rise.rise = mod(rise.rise + zone, 24.0); // correct for time zone, if time is valid
            }
            if (rise.transit != 0.000000000) {
                rise.transit = mod(rise.transit + zone, 24.0); // correct for time zone, if time is valid
            }
            if (rise.set != 0.000000000) {
                rise.set = mod(rise.set + zone, 24.0); // correct for time zone, if time is valid
            }
        }
        return rise;
    }
    ...
    public static class Time {
        ...
        public Time(double hhi) {
            ...
            hhmmssStringdec = hhmmssString + " = " + StringDec(hhi, 5);
            hhmmStringdec = hhmmString + " = " + StringDec(hhi, 5);
        }
